...
function plot_mnist_image(
    adv_example, clean_example, clean_pred_label, adv_predicted_label, true_label
)
...
adversarial image: ",
    ...
    push!(
        ...
        heatmap(
            ...
            color=:grays,
            axis=false,
            title="Clean example: $true_label -> $clean_pred_label",
        ),
    )
    push!(
        hms,
        heatmap(
            ...
            color=:grays,
            axis=false,
            title="Adv example: $true_label -> $adv_predicted_label",
        ),
    )
    return plot(hms...; layout=(1, 2), colorbar=false)
end

function plot_mnist_clean_and_attack(
    x, y, attack, model, loss, ϵ; step_size=0.01, iterations=10, attack_method=:FGSM
)
    ...
    if attack_method == :FGSM
        adversarial_example = attack(
            model, loss, clean_example, onehotbatch(actual, 0:9); ϵ=ϵ
        )
    ...
    clean_prediction = (getindex(Flux.onecold(model(clean_example)))) - 1
    adversarial_prediction = (getindex(Flux.onecold(model(adversarial_example)))) - 1

    return plot_mnist_image(
        adversarial_example, clean_example, clean_prediction, adversarial_prediction, actual
    )
end
...
function plot_normal_and_ce(real_image, real_pred, cf_image, cf_pred, actual)
    ...
    hms = []
    push!(
        hms, heatmap(clean_img_matrix; color=:grays, axis=false, title="Clean: $real_pred")
    )
    push!(
        hms,
        heatmap(image_matrix; color=:grays, axis=false, title="Counterfactual: $cf_pred"),
    )
    return plot(hms...; layout=(1, 2), colorbar=false)
end
