...
function evaluate_model(
    ...
    for index in 1:num_datapoints
        clean_example = x_test[:, index]
        actual = y_test[index]

        clean_pred = (getindex(Flux.onecold(softmax(model(clean_example))))) - 1
        ...
        if attack_method == :FGSM
            adversarial_example = attack(
                ...
                loss,
                clean_example,
                onehotbatch(actual, label_min:label_max);
                ϵ=ϵ,
        ...
        adv_pred = (getindex(Flux.onecold(softmax(model(adversarial_example))))) - 1
