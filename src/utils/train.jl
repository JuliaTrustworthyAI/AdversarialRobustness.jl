...
function vanilla_train(model, loss, opt, x_train, y_train, max_epochs, batch_size, label_min, label_max)
    θ = Flux.params(model)
    vanilla_losses = []
...
function adversarial_train(model, loss, opt, x_train, y_train, epochs, batch_size, attack, label_min, label_max, ϵ; step_size = 0.01, iterations = 10, attack_method = :FGSM, clamp_range=(0, 1))
    adv_losses = []
    θ = Flux.params(model)
    train_loader = DataLoader((x_train , y_train), batchsize=batch_size, shuffle=true)
    ...
    @showprogress for epoch in 1:epochs
        ...
        for (idx, (x, y)) in enumerate(train_loader)
            ...
            elseif attack_method == :PGD
                x_adv = attack(model, loss, x, y_onehot; ϵ=ϵ, step_size=step_size, iterations=iterations, clamp_range=clamp_range)
            else 
